defmodule Server do
    def startServer() do
      Task.start(fn -> Twitter.Server.start_link() end)
      # receive do
      #   {_} -> :ok
      # end
    end
  ...
  defmodule Twitter.Server do
    ...
    #   Node.set_cookie(serverNodeName,:twitter)
      ...
      :global.register_name(:ServerProcess,serverpid)
    ...
    def initialize_tables() do
      ...
      :ets.new(:usersSock,[:set, :public, :named_table])
      ...
      :ets.insert(:tweetsCount,{:tweetsCount,[0,200]})
    ...
    #   :ets.insert(:tweetsCount,{:tweetsCount,[count,tweetsCount]})
      ...
      Enum.each(list,fn(x)->
        if isUserOnline?(x) do
          ...
          sockpid = elem(Enum.at(:ets.lookup(:usersSock,x),0),1)
